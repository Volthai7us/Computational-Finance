...
def get_portfolio_details():
    ...
    for i in range(num_options):
        ...
        option_price = 0
        # if auto_option_pricing:
        #     option_price = Utils.get_option_price(
        #         OptionType.CALL if option_type == "CALL" else OptionType.PUT, spot_price, strike_price)
        #     st.write(f"Opsiyon fiyatı: {option_price}")
...
if selection == "Example Portfolios":
    ...
    portfolios = get_example_portfolios(risk_free_rate, volatility, time)
    ...
    num_options, num_stocks, stock_price, option_details, risk_free_rate, volatility, time, auto_option_pricing = get_portfolio_details()
